...
from titans.layer.block import DeepNetTransformerLayer1D, FusedDeepNetTransformerLayer1D
from titans.layer.embedding import HiddenParallelEmbedding, VocabParallelEmbedding
from titans.layer.head import VocabParallelGPTLMHead1D as VocabParallelLMHead1D, HiddenParallelGPTLMHead1D as HiddenParallelLMHead1D

__all__ = ['GenericPipelineDeepNet', 'PipelineDeepNet1D', 'FusedPipelineDeepNet1D', 'deepnet_small_pipeline_1D']
...
class PipelineDeepNet1D(GenericPipelineDeepNet):
    ...
    def __init__(self,
        ...
        blocks = nn.ModuleList([
            DeepNetTransformerLayer1D(hidden_size,
                                      num_attention_heads,
                                      act_func=act_func,
                                      mlp_ratio=mlp_ratio,
                                      ...
                                      alpha=alpha) for _ in range(num_layers)
        ])
        if last:
...
class FusedPipelineDeepNet1D(GenericPipelineDeepNet):
    ...
    def __init__(self,
        ...
        if first:
            embedding = embed_cls(hidden_size, vocab_size, max_position_embeddings, embed_drop_rate, dtype=dtype)
        blocks = nn.ModuleList([
            FusedDeepNetTransformerLayer1D(hidden_size,
                                           num_attention_heads,
                                           act_func=act_func,
                                           mlp_ratio=mlp_ratio,
                                           attention_dropout_prob=attn_drop_rate,
